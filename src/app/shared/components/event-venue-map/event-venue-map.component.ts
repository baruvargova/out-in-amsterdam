...
export class EventVenueMapComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.venues) {
      this.venueMarkers = this.venues.map(venue => new EventVenueMarker(true, venue.trcid, venue.title, venue.location, venue.urls, this.getVenueMarkerOption(venue)));
    }
    if (changes.events) {
      this.eventMarkers = this.events.map(event => new EventVenueMarker(false, event.trcid, event.title, event.location, event.urls, this.getEventMarkerOption(event)));
    }
  }
  ...
  public openInfoWindow(marker: MapMarker, eventMarker: EventVenueMarker) {
